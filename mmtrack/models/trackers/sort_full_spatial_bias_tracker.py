        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if self.with_reid:
                ...
                if len(active_ids) > 0:
                    ...
                    col = spatial_bias_assignment(reid_dists).tolist() # indices of col represent row indices
                    row = list(range(len(col)))
                    ass_dists = reid_dists[row,col]
                    is_finite = not np.isfinite(ass_dists) 
                    is_below = ass_dists <= self.reid["match_score_thr"]
                    is_cond_met = (is_finite * is_below) # is condition met?
                    nnz_indices = np.flatnonzero(is_cond_met).tolist()
            ...
            if len(active_ids) > 0:
                ...
                dists = 1 - ious
                col = spatial_bias_assignment(dists).tolist() # col indice is equivalent to row indices
                row = list(range(len(col)))
                ass_bbox_dists = dists[row,col] # associated bounding box distances
                nnz_indices = np.flatnonzero(ass_bbox_dists < (1-self.match_iou_thr)).tolist()
                if len(nnz_indices) != 0:
                    for nnz_idx in nnz_indices:
                        r = row[nnz_idx]
                        c = col[nnz_idx]
